describe('Reservation', () => {
  it('Make a successfuly reservation', () => {
    ...
    cy.get('.MuiDayPicker-monthContainer > :nth-child(5) > :nth-child(1)').click();
  ...
  it('Make a reservation with booking day before current day', () => {
    ...
    cy.get('.Toastify__toast-body > :nth-child(2)').should('have.text', 'You are picking a Date that not available. Date will be set to Today');
    /* ==== End Cypress Studio ==== */
  })
  it('Make a reservation with guest is equal to 0', () => {
    ...
    cy.get('.bg-neutral-200 > .absolute').should('have.text', '*Guest must be greater than 0');
    /* ==== End Cypress Studio ==== */
  })
  ...
  it.only('Make a reservation without choose day and number of guest', () => {
    ...
    cy.get('.bg-second > .MuiButtonBase-root').click();
    cy.get('#date\\ must\\ be\\ a\\ valid\\ ISO\\ 8601\\ date\\ string').should('be.visible');
    /* ==== End Cypress Studio ==== */
  })
})
