...
/**
 * * This api is used to generate a sentence randomly
 * * Use for develop chatbot
 */
export default function handler(
  req: NextApiRequest,
  res: NextApiResponse<Data>
) {
  const randomIdx = Math.floor(Math.random() * sentences.data.length);
  const generatedSentence = sentences.data[randomIdx];
  res.status(200).json(generatedSentence);
}
